...
import (
	...
	"errors"
	"fmt"
	http2 "github.com/Kora-Dance/koradance-backend/app/helper/http"
	"io/ioutil"
	"log"
	"net/http"
	"net/http/httputil"
	...
	return func(c *gin.Context) {
		...
		defer func() {
			if !recoveryBreak() {
				...
				if r != nil {
					publishError(r, request, true)
					errResp := http2.ErrorResponse{
						Code:       http2.StatusServerError,
						ErrMessage: "Terjadi kesalahan, harap coba lagi nanti",
						ErrReason:  r.Error(),
					}
					http2.WriteErrorResponseObj(c, time.Now(), http2.StatusServerError, errResp)
				}
			}
		}()
